...
import com.mashen.datatables.domain.Column;
import com.mashen.datatables.domain.DataTablesRequest;
import com.mashen.datatables.domain.ExampleProxy;
import com.mashen.datatables.domain.Order;
...
public class SearchConditionUtils {
	 ...
	 * 必填查询条件判断
	 */
	public static void validSearchCondition(Map<String,Object> searchColumns,String... condition){
		for(String key : condition){
			if(searchColumns.get(key)==null||"".equals(searchColumns.get(key))){
				 new RuntimeException("search column "+key+"is empty");
			}
		}
	}
	
	public static <T> void wrapperAndCondition(T example,DataTablesRequest request) throws Throwable{
		// 注入查询条件
		...
		//封装排序条件
		String orderByClause=SearchConditionUtils.getOrderString(request.getColumns(),request.getOrder());
	...
	public static <T> void wrapperAndCondition(T example,PageVO pagevo,Map<String,Object> conditionMap) throws Throwable{
		// 注入查询条件
		ExampleProxy<T> exampleProxy = new ExampleProxy<T>(example);
		//设置分页参数
	...
	private static void setCondition(Object criteria,HashMap<String, Method> criteriaMethodMap,Map<String,Object> conditionMap) throws Throwable{
		for(Map.Entry<String,Object> entry : conditionMap.entrySet()){
			Method method=criteriaMethodMap.get("and"+entry.getKey());//不出and
